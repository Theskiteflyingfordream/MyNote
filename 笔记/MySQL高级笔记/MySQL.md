...
/*Note:系统变量永久生效需要加在配置文件中，否则重启mysql服务即恢复默认*/
...
- `DERIVED`：在`FROM`子句中包含的子查询被标记为`DERIVED(衍生)`，MySQL会递归执行这些子查询，把结果放在临时表中。
  ...
  /*Note：table字段显示derivedn表示from id为n的结果*/
...
/***Note：详细解析见**[MySQL Explain命令详解：type列详解及案例分析 - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/358920539)*/
...
/***NOTE：覆盖索引即select后的字段均在索引结点中出现（无论顺序），不用查询数据表。Myisam与inonodb的覆盖索引不同，前者索引节点中没有主键，如果需要查主键，要回表查找***/
...
/***Note:连接的原理以及索引的实现原理在浏览器收藏***/



# 10.索引失效（注意mysql一张表只能使用一个索引，但是三表连接等中可以使用多个索引）
...
***select id,name,date from table where id=10001 and name>‘Assistant’ and date=‘2001-09-03’;条件从左到右依次为复合索引
范围索引失效的原因：先看看name为什么生效，name>‘Assistant’ 也就是说在id=10001的情况下，只需要用指针按顺序找到name为’Assistant’ 右边的所有数就行，所以它生效了；至于第三个date=‘2001-09-03’，大家想想，只有在第一层，第二层固定都相等的情况下，第三层才会排序，才能按照顺序把第三个值找出来，也就是说，第二层条件如果是个范围，那么第三层就是无序的，那么系统该怎么去找出第三层指定的值呢，只有在范围后进行全盘扫描一个个找才能找到指定的值，因此出现了第三层的索引失效***
...
1.isnull 和isnotnull索引失效，isnull可以对null添加默认值

2.应尽量避免在 where 子句中使用!=或<>操作符，否则引擎将放弃使用索引而进行全表扫描（**主键除外，主键是用索引的**）。

3.应尽量避免在 where 子句中使用or 来连接条件，否则将导致引擎放弃使用索引而进行全表扫描（可用union）（**主键除外，主键是用索引的**）
...
**当比较不同类型的操作数时，会隐式发生类型转换，自动地把字符串型转成数字。**

**隐式转化把字符串转为了整数类型，所以索引是数值，查询传入字符还是会走索引。**

**而索引是字符，查询传入数值，索引会发生隐式转换，导致索引失效。**
...
/* 8.没有使用索引进行排序 产生了Using filesort ,注意理解此例子*/
...
- 尝试提高`max_length_for_sort_data`：提高这个参数，会增加用单路排序算法的概率。但是如果设置的太高，数据总容量超出`sort_buffer_size`的概率就增大（合适的max_length_for_sort_data能使在数据总容量过高时使用双路排序，较低时使用单路排序），明显症状是高的磁盘IO活动和低的处理器使用率。
...
- `long_query_time`的默认值为10，意思是运行超过10秒的语句。
...
# 2、开启慢查询日志
mysql> SET GLOBAL slow_query_log = 1;	/*Note:只对当前数据库有效，mysql重启后失效*/
Query OK, 0 rows affected (0.00 sec)
```

/*Note：开启时报错，但是文件已创建，此时是mysql用户不具有操作此文件的权限，应当chown mysql 文件*/
...
**/*Note：也可以用set global，但是需要重新连接才能看到效果，且当前连接使用不生效*/**
...
/*Note: select sleep(n)可以让sql语句执行n秒*/
...
> 2、由于开启开启了`bin-log`，我们就必须为`function`指定一个参数，否则使用函数会报错。
>
> ​		[MySQL参数log_bin_trust_function_creators介绍 - 潇湘隐者 - 博客园 (cnblogs.com)](https://www.cnblogs.com/kerrycode/p/7641835.html)
...
/*Note:对于用户变量，调用存储过程或者函数时，若之前没有声明，参数列表中直接传入@变量名即可*/
...
此后，编写的sql语句将会被记录到mysql库里的general_log表中
...
```mysql
# 释放给表添加的锁（Note：释放当前线程的所有）
UNLOCK TABLES;

# 查看当前表的状态（Note：不同线程的锁也将被显示）
mysql> SHOW OPEN TABLES;
+--------------------+------------------------------------------------------+--------+-------------+
| Database           | Table                                                | In_use | Name_locked |
+--------------------+------------------------------------------------------+--------+-------------+
...
- 対`MyISAM`表的读操作（加读锁），不会阻塞其他线程対同一表的读操作（可同时加多个读锁？），但是会阻塞其他线程対同一表的写操作，同时当前线程无法修改加锁的表。只有当读锁释放之后，才会执行其他线程的写操作。
- 対`MyISAM`表的写操作（加写锁），会阻塞其他线程対同一表的读和写操作，只有当写锁释放之后，才会执行其他线程的读写操作。

Note：当前线程加了读锁，可以继续加写锁，而其它线程加写锁，则被阻塞；

同一个线程只能一次加锁（用逗号分割），若再加锁，之前加的会被释放；
...
/***Note:没有索引或者索引失效时，InnoDB 的行锁变表锁，原因是mysql的行锁是通过索引实现的，而进行全表扫描则变成表锁***/
...
当我们用范围条件而不是相等条件检索数据，并请求**共享或者排他锁时，`InnoDB`会给符合条件的已有数据记录的索引项加锁，对于键值在条件范文内但并不存在的记录，叫做"间隙(GAP)"。（注意请求共享锁也会导致间隙锁）**

`InnoDB`也会对这个"间隙"加锁，这种锁的机制就是所谓的"间隙锁"。

/***Note:间隙锁案例解析**[mysql间隙锁 - 无名草110 - 博客园 (cnblogs.com)](https://www.cnblogs.com/billmiao/p/9872161.html)*/
...
/***Note：注意从机不会包含有主机主从复制前的数据，如果从机需要这些数据，需要将其打包发送到从机***/
...
/***Note：windows下找mysql安装目录以及数据目录(配置文件所在)[windows下的mysql配置文件在哪 - 张彦山 - 博客园 (cnblogs.com)](https://www.cnblogs.com/blogzys/p/10038377.html)***/
...
[mysqld]
server-id=1 # 必须
log-bin=/var/lib/mysql/mysql-bin # 必须	/*window在数据目录下*/
read-only=0
...
# 3、查看Slave状态
# Slave_IO_Running 和 Slave_SQL_Running 必须同时为Yes 说明主从复制配置成功！否则需要从机停止主从复制再进行配置
